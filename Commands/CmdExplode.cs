...
namespace MCForge
{
    class CmdExplode : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (message == "me")
            {
                if (p == null) { Player.SendMessage(p, "Console cannot explode."); return; }
            ...
            if (number == 1)
            {
                ...
                if (who != null)
                {
                    if (who.level.physics < 3)
                    {
                        Player.SendMessage(p, "The physics on their level are not sufficient for exploding!");
                        return;
                    }
            ...
            if (number == 3)
            {
                if (p == null) { Player.SendMessage(p, "Console is not in a level."); return; }
